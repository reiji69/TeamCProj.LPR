...
while True:
    ...
    def myfunction(event):
        ...
        if canvas.old_coords:
            ...
            canvas.create_oval(x-2, y-2, x1+2, y1+2, fill = "black")
            a[y // 10][x // 10] = 1
            """
            a[y // 10 - 1][x // 10] = 25
            a[y // 10 - 1][x // 10] = 25
    ...
    def save():
        ...
        for i in range(28):
            for j in range(28):
                c.append(a[i][j])
        d.append(c)
        d = np.asarray(d, dtype = None, order = None)
        root.destroy()
        #p = sess.run(pred, feed_dict={x: d, keep_prob: 1.0})
        p = sess.run(tf.argmax(pred,1), feed_dict={x: d, keep_prob: 1.0})[0]
        print(p)
        print(chr(p+65))
    ...
    canvas.pack()
    canvas.old_coords = None
    canvas.bind("<B1-Motion>", myfunction)
    canvas.bind("<ButtonRelease-1>", myfunction2)
